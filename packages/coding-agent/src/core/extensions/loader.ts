...
/** Bundled modules available to extensions in source, built Node, and compiled Bun runtimes. */
...
async function loadExtensionModule(extensionPath: string, cacheToken?: ExtensionCacheToken) {
	...
	const jiti = createJiti(import.meta.url, {
		...
		// Source TypeScript additionally reuses root tsconfig paths. Bun must not
		// fall back to native resolution for modules embedded in the executable.
		...(isBunBinary ? { tryNative: false } : isTypeScriptSourceRuntime ? { tsconfigPaths: true } : {}),
	});
